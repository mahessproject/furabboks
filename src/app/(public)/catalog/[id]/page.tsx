...
import { ArrowLeft, BookOpen, Lock, Star, User } from "lucide-react";
...
export default async function PublicBookDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loginUrl = `/?auth=login&next=/books/${id}`;
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      ...
      <div className="mt-6 grid grid-cols-1 gap-8 sm:grid-cols-[240px_1fr]">
        {/* Cover */}
        <div className="relative aspect-2/3 w-full max-w-60 overflow-hidden rounded-lg border border-violet-200 bg-violet-50">
          {book.cover_url ? (
        ...
        <div className="space-y-4">
          ...
          <p className="text-sm text-violet-600">
            oleh <span className="font-medium text-violet-700">{book.author}</span>
          </p>
          ...
          {book.description && (
            <p className="text-sm leading-relaxed text-violet-600">{book.description}</p>
          )}

          <div className="border-t border-violet-200 pt-4">
            <p className="text-2xl font-bold text-violet-900">{formatRupiah(Number(book.price))}</p>
            <p className="mt-1 text-sm text-violet-600">
              {book.stock > 0 ? (
                <span className="text-green-600">Stok tersedia ({book.stock})</span>
              ) : (
                <span className="text-violet-600 font-bold">Stok habis</span>
              )}
            </p>
          </div>

          {/* Login CTA */}
          <div className="space-y-3 rounded-xl border border-violet-100 bg-violet-50 p-4">
            <div className="flex items-center gap-2 text-sm font-medium text-violet-700">
              <Lock className="h-4 w-4" />
              Login diperlukan untuk membeli buku ini
            </div>
            <p className="text-xs text-violet-600">
              Buat akun gratis atau masuk untuk menambahkan buku ke keranjang.
            </p>
            <div className="flex gap-2">
              ...
              <Link
                href="/catalog"
                className="rounded-md border border-violet-300 bg-white px-4 py-2 text-sm font-medium text-violet-700 hover:bg-violet-100"
              >
                Kembali ke Katalog
              </Link>
            </div>
        ...
        {reviews.length === 0 ? (
          ...
          <div className="space-y-3">
            {reviews.map((review) => {
              ...
              return (
                <div key={review.id} className="rounded-xl border border-violet-200 bg-white p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="text-xs text-violet-400 shrink-0">{formatDate(review.created_at)}</span>
                  </div>
                  {review.comment && (
